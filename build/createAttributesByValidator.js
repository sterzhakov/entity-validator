const createAttributesByValidator = (validator, params) => {
  ...
  const attributes = templateAttributes.map(templateAttribute => {
    const rules = templateAttribute.rules.map(templateAttributeRule => {
      if (Array.isArray(templateAttributeRule)) {
        ...
        throw new Error(`Unsupported type ${attributeType} for template attribute rule ` + `${templateAttributeRule.key}.`);
      }
    });
    return {
      key: templateAttribute.key,
      rules
    };
  });
  return attributes;
