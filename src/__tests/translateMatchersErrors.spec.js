// @flow
import type { MatcherError } from '../types';
...
describe('translateMatchersErrors', () => {
  it('add translation to matcher errors', () => {
    const matchersErrors: Array<MatcherError> = [
      {
        key: 'presence',
        attribute: 'name',
    ...
    const translate = (key, params = {}) => {
      ...
      const dictionary = {
        presence: ({ name }) => `Hello ${name}!`,
      };
      return dictionary[key](params);
    };

    const translatedMatcherErrors =
      translateMatchersErrors(matchersErrors, {
        key: 'message',
        translate,
        params: {name: 'world'},
      });

    expect(translatedMatcherErrors).toEqual([
      {
        ...matchersErrors[0],
        message: 'Hello world!',
      },
